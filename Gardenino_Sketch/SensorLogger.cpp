...
SensorLogger::SensorLogger(Clock* logClock) : PrintLogSpan(0, 1, 0, 0), LogClock(logClock)
{
}
...
void SensorLogger::Log()
{
	...
	String FileName = LOGS_FOLDER + String("/") + String(LastLogTime.day()) + String(LastLogTime.month()) +
					  String(LastLogTime.year()) + String(".csv");
	...
	String newLine = String(LastLogTime.hour()) + String(":") + String(LastLogTime.minute()) + String(":") +
					 String(LastLogTime.second()) + ";";
	...
	SERIAL_LOG("[" + String(LastLogTime.hour()) + ":" + String(LastLogTime.minute()) + ":" +
			   String(LastLogTime.second()) + "] Logged data to sd")
}